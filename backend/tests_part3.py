...
def test_admin_update_user_role():
    ...
    if response.status_code == 200:
        assert "עודכן" in response.json()["message"]


def test_admin_delete_user():
    response = client.delete("/admin/users/99")  # נסה עם ID זמני שלא קיים
    assert response.status_code in [200, 404]
...
def test_admin_update_exhibition():
    ...
    assert response.status_code in [200, 404]
    if response.status_code == 200:
        assert "עודכן" in response.json()["message"] or "התערוכה עודכנה" in response.json()["message"]
